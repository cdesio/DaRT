...
int main(int argc, char **argv)
{
  ...
  if (((commandLine = parser->GetCommandIfActive("-decayPS"))) || ((commandLine = parser->GetCommandIfActive("-photonPS"))))
  {
    ...
    if ((commandLine = parser->GetCommandIfActive("-decayPS")))
    {
      ps_file.seekg(0, ps_file.end); // change position to end 
      PS_data = ps_file.tellg()/(12*8); //12 doubles which are each 8 bytes

    }
    else
    {
      ps_file.seekg(0, ps_file.end); // change position to end 
      PS_data = ps_file.tellg()/(8*8); //8 doubles which are each 8 bytes
    }
  }
  ...
  if (((commandLine = parser->GetCommandIfActive("-decayPS")))||((commandLine = parser->GetCommandIfActive("-photonPS"))))
  {
    ...
    G4cout << "number of beams = " << PS_data << G4endl;
...
void Parse(int &argc, char **argv)
{
  ...
  parser->AddCommand("-histone",
                     ...
                     exec);
  parser->AddCommand("-decayPS",
                     Command::WithOption,
                     "PS file from decay simulaiton");
  parser->AddCommand("-photonPS",
                     Command::WithOption,
                     "PS file from photon simulaiton");
  //////////
