...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  // G4double wirePosition = 20*um; 
  // G4double wireRadius = 10*um;
  // G4double zPos = (G4UniformRand() - 0.5) * wirePosition;
  // G4double DummyAngle = G4UniformRand() * 2 * 3.1415926535;
  // G4double depth = 0; //depth reported to be 5-20 nm deep, but recoil in Geant4 is only 1nm (geometric distance) so the Radon would never leave the source. Reported desorption percentages are 40% for Rn and 55% for Pb. Depth chosen as a compromise between these values.
